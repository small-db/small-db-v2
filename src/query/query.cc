...
arrow::Status RunSubstraitConsumer() {
    ...
    ARROW_ASSIGN_OR_RAISE(auto plan_json, eng::internal::SubstraitToJSON(
    ...
    // The data sink(s) for plans is/are implicit in substrait plans, but
    // explicit in Arrow. Therefore, deserializing a plan requires a factory for
    // consumers: each time the root of a substrait relation tree is
    // deserialized, an Arrow consumer is constructed into which its batches
    // will be piped.
    std::vector<std::shared_ptr<ac::SinkNodeConsumer>> consumers;
    std::function<std::shared_ptr<ac::SinkNodeConsumer>()> consumer_factory =
        [&] {
            // All batches produced by the plan will be fed into
            // IgnoringConsumers:
