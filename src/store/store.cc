...
namespace store {
    ...
    string gen_datafile_path(const string& tablename) {
        ...
        auto now_ns =
        chrono::duration_cast<chrono::nanoseconds>(now.time_since_epoch()).count() % 1000000000;
    ...
    arrow::Status execute_plan_and_collect_as_table(arrow::acero::Declaration plan) {
        ...
        response_table, arrow::acero::DeclarationToTable(std::move(plan)));
    ...
    arrow::Status scan_sink_example(shared_ptr<arrow::Table> table) {
        ...
        options->projection = arrow::compute::project({}, {}); // create empty projection
    ...
    void init_schemas() {
        ...
        { arrow::field("name", arrow::utf8()), arrow::field("tables", type_tables) });
        ...
        arrow::MapBuilder tables_builder(
        arrow::default_memory_pool(), keys_builder, values_builder);
        ...
        arrow::io::FileOutputStream::Open(gen_datafile_path(TABLE_SCHEMAS)));
    ...
    void init_tables() {
        ...
        std::vector<std::shared_ptr<arrow::ArrayBuilder>> field_builders = { column_names_builder,
            column_types_builder };
        auto column_builder =
        ...
        // create the table
        std::shared_ptr<arrow::Schema> schema = arrow::schema(
        { arrow::field("name", arrow::utf8()), arrow::field("columns", type_columns) });
        auto table = arrow::Table::Make(schema, { table_names, columns_list });

        // write to disk
        std::shared_ptr<arrow::io::FileOutputStream> outfile;
        PARQUET_ASSIGN_OR_THROW(outfile,
        arrow::io::FileOutputStream::Open(gen_datafile_path(TABLE_TABLES)));
        PARQUET_THROW_NOT_OK(parquet::arrow::WriteTable(
        *table, arrow::default_memory_pool(), outfile, 300));
    ...
    void init_demo() {
        ...
        // write to disk
        std::shared_ptr<arrow::io::FileOutputStream> outfile;
        PARQUET_ASSIGN_OR_THROW(outfile, arrow::io::FileOutputStream::Open("demo.parquet"));
        PARQUET_THROW_NOT_OK(parquet::arrow::WriteTable(
        *table, arrow::default_memory_pool(), outfile, 300));
