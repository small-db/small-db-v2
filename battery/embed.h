...
struct EmbedInternal {
  ...
  public:
    ...
    constexpr EmbeddedFile(const std::string_view &data,
                           const std::string_view &filename)
        : m_data(data), m_filename(filename) {}

    [[nodiscard]] std::string str() const { return m_data.data(); }
    ...
    void get(const std::function<void(const b::EmbedInternal::EmbeddedFile &)>
                 &callback);
...
template <size_t N> struct embed_string_literal {
  ...
  [[nodiscard]] std::string str() const { return std::string(value, N); }
  [[nodiscard]] constexpr bool _false() const { return false; }
  char value[N];
};

template <size_t N, size_t M>
constexpr bool operator==(const embed_string_literal<N> &left,
                          const char (&right)[M]) {
...
constexpr EmbedInternal::EmbeddedFile embed() {
  if constexpr (identifier == "resources/banner.txt") {
    ...
    static_assert(identifier._false(),
